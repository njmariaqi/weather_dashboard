...
function searchCity() {
     document.addEventListener("click", function(event) {
          ...
          if (element.matches("#searchBtn")) {
               ...
               getCoord();
               if (cityHistory.includes(cityInput.value.toUpperCase()) === false) {
                    console.log("not repeat");
                    cityHistory.unshift(cityInput.value.toUpperCase());
                    localStorage.setItem("history", JSON.stringify(cityHistory));
                    var currentSearch = document.createElement("button");
                    currentSearch.textContent = cityInput.value.toUpperCase();
